node {
	...
	stage('SonarQube analysis'){
		...
		powershell '''
			...
			.\\sonar-scanner.bat -X -D sonar.projetKey=onePipeline -D sonar.projectName=DotNetHello -D sonar.projectVersion=2.0 -D sonar.login=admin -D sonar.password=admin -D sonar.sources=Hello\\hello.sln
		'''
		}
	}
}